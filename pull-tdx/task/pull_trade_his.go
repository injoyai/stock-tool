...
func NewPullTradeHistory(codes []string, dir string, limit int) *PullTradeHistory {
	return &PullTradeHistory{
		Dir:   dir,
		Limit: limit,
		Codes: codes,
	}
...
type PullTradeHistory struct {
	Dir   string
	Limit int
	Codes []string
}

func (this *PullTradeHistory) Name() string {
	return "历史分时成交"
}

func (this *PullTradeHistory) Run(ctx context.Context, m *tdx.Manage) error {
	r := &Range[string]{
...
func (this *PullTradeHistory) Handler(ctx context.Context, m *tdx.Manage, code string) error {
	for i := 2000; i <= time.Now().Year(); i++ {
		...
		if err := this.Pull(m, i, code); err != nil {
			return err
		}
	}
	return nil
...
func (this *PullTradeHistory) Pull(m *tdx.Manage, year int, code string) (err error) {
	...
	err = this.TradeToCsv(filename, tss)
	if err != nil {
		return err
	}

	err = this.KlinesToCsv(filename1, code, name, kss1)
	if err != nil {
		return err
	}

	err = this.KlinesToCsv(filename5, code, name, kss5)
	if err != nil {
		return err
	}

	err = this.KlinesToCsv(filename15, code, name, kss15)
	if err != nil {
		return err
	}

	err = this.KlinesToCsv(filename30, code, name, kss30)
	if err != nil {
		return err
	}

	err = this.KlinesToCsv(filename60, code, name, kss60)
	if err != nil {
		return err
	}
...
func (this *PullTradeHistory) KlinesToCsv(filename string, code, name string, ks protocol.Klines) error {
...
func (this *PullTradeHistory) TradeToCsv(filename string, ts protocol.Trades) error {
